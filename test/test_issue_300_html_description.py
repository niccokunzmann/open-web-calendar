...
def test_description_is_html(calendar_content):
    ...
    event = event_with_uid(calendar_content["issue-287-links-1"], "1jkma74p98bj79c760660arc07@google.com")
    description = ConvertToDhtmlx({"timezone":"Europe/London"}).get_event_description(event)
    assert '<a href="https://bethanymarceline.com/"' in description


def test_description_is_parameter(calendar_content):
    """The description of the event is already in HTML."""
    event = event_with_uid(calendar_content["event-with-html-markup"], "683642b3-9b25-4177-8b46-ec2f65e64020")
    description = ConvertToDhtmlx({"timezone":"Europe/London"}).get_event_description(event)
    assert '<h1>\n Know This Heading!\n</h1>' in description
