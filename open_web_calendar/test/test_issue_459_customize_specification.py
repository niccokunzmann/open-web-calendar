...
import json
from open_web_calendar.app import get_default_specification, DEFAULT_SPECIFICATION
import pytest
import os
...
def test_values_from_specification_file(monkeypatch):
...
@kv
def test_we_can_load_the_spec_from_a_file(tmp_path, monkeypatch, k, v):
    ...
    assert get_default_specification()[k] == "other value", "The value in the app dict are more important."
...
def test_an_empty_spec_is_alrigth(monkeypatch):
    """The spec variable must be ok to be empty."""
    monkeypatch.setitem(os.environ, "OWC_SPECIFICATION", "")
    get_default_specification()
    monkeypatch.delitem(os.environ, "OWC_SPECIFICATION")
    get_default_specification()
