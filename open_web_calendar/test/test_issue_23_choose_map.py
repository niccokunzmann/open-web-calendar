...
def test_event_in_mountain_view(client : FlaskClient, calendar_urls):
    ...
    event = events[0]
    assert event["location"]["text"] == "Mountain View, Santa Clara County, Kalifornien, Vereinigte Staaten von Amerika"
...
def test_event_in_berlin_with_altrep(client : FlaskClient, calendar_urls):
    ...
    url : str = event["location"]["url"]
    assert url == "https://www.openstreetmap.org/relation/62422", "should be the same url as in altrep"
